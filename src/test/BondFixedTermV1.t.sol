...
contract BondFixedTermV1Test is Test {
    ...
    // Can't reasonably test this with new guard rails in place on tuning
    function testCorrectness_StopControlAdjustment(
        ...
        uint256 amount = auctioneer.maxAmountAccepted(id, referrer);
        uint256 fee = amount.mulDiv(teller.getFee(referrer), 1e5);
        uint256 minAmountOut = (amount - fee).mulDiv(scale / 100, price);
        ...
        amount = auctioneer.maxAmountAccepted(id, referrer);
        fee = amount.mulDiv(teller.getFee(referrer), 1e5);
        price = auctioneer.marketPrice(id);
        minAmountOut = (amount - fee).mulDiv(scale / 100, price);
        vm.prank(alice);
